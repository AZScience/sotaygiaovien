...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState<CurrentUser | null>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('sotay_giaovien_currentUser');
      if (saved) {
        try {
  ...
  const handleLogout = () => {
    setCurrentUser(null);
    localStorage.removeItem('sotay_giaovien_currentUser');
  };

  const handleLogin = (email: string, password: string) => {
    // 1. Hardcoded admin fallback for bootstrap
    if ((email === 'admin' || email === 'admin@example.com') && (password === 'admin123' || password === 'admin')) {
      const user: CurrentUser = { id: 'admin_sys', email: 'admin@example.com', role: 'admin', name: 'Quản trị viên Hệ thống' };
      setCurrentUser(user);
      localStorage.setItem('sotay_giaovien_currentUser', JSON.stringify(user));
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden print:h-auto print:overflow-visible print:bg-white">
        ...
        <header className="bg-white border-b border-slate-200 px-4 md:px-6 py-3.5 md:py-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4 shrink-0 print:hidden shadow-xs">
          ...
          <div className="flex items-center gap-3 lg:ml-auto">
            ...
            <div className="flex flex-wrap gap-1 bg-slate-100 p-1 rounded-xl min-w-0 max-w-full">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 transition cursor-pointer text-slate-700"
                title="Tài khoản"
              >
                <div className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center shrink-0">
                  <User className="w-3.5 h-3.5" />
                </div>
                <span className="text-xs font-bold truncate max-w-[100px] hidden md:block">
                ...
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-lg border border-slate-200 py-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 flex items-center gap-2 transition cursor-pointer"
                    >
